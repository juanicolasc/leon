class Charge < ApplicationRecord
    ...
    def self.count_search(start_date, end_date)
        select('sum(charges.price - items.cost) as total_profit, sum(items.cost) as total_cost, sum(items.price) as total_retail, sum(charges.price) as total_sell , count(*) as total_quantity').joins(:sale, :item).where("sales.date" => start_date..end_date).order('total_profit DESC') 
    end
